...
def setup_uv_environment():
    ...
    if tvenv_path.exists():
        ...
        if not run_command(
            ["uv", "venv", ".tvenv", "--python", "3.13"],
            "Creating .tvenv virtual environment",
        ):
            return False
...
Examples:
  ...
  python run_tests.py --keep-env
    ...
    parser.add_argument(
        "--shared-only", action="store_true", help="Run only shared module tests"
    )
    ...
    parser.add_argument(
        "--exclude-slow", action="store_true", help="Exclude slow tests"
    )
    ...
    parser.add_argument(
        "--clean", action="store_true", help="Clean up test artifacts before running"
    )
    ...
    for app_dir in app_dirs:
        ...
        if app_dir_path.is_dir():
            try:
                ...
                print(
                    f"⚠️  Warning: Could not remove application directory {app_dir}: {e}"
                )
