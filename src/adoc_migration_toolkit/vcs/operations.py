...
def execute_vcs_config(command: str) -> bool:
    ...
    try:
        ...
        (
            ...
            proxy_username,
            proxy_password,
        ) = parse_vcs_config_command(command)
...
def collect_vcs_config_interactive(
    existing_config: Optional[VCSConfig] = None,
) -> Optional[VCSConfig]:
    ...
    try:
        # VCS Type
        vcs_type = input(
            f"VCS type (git, hg, svn) [{existing_config.vcs_type if existing_config else 'git'}]: "
        ).strip()
        if not vcs_type:
        ...
        # Remote URL
        remote_url = input(
            f"Remote repository URL [{existing_config.remote_url if existing_config else ''}]: "
        ).strip()
        if not remote_url:
        ...
        auth_choice = (
            input(f"Authentication method (https/ssh) [{detected_auth}]: ")
            .strip()
            .lower()
        )
        if not auth_choice:
        ...
        if auth_method == "https":
            ...
            username = input(
                f"Username [{existing_config.username if existing_config else ''}]: "
            ).strip()
            if not username:
            ...
            ssh_key_path = input(
                f"SSH key path [{existing_config.ssh_key_path if existing_config else '~/.ssh/id_rsa'}]: "
            ).strip()
            if not ssh_key_path:
                ssh_key_path = (
                    existing_config.ssh_key_path if existing_config else "~/.ssh/id_rsa"
                )

            # Always ask for SSH key path, even if empty
            if not ssh_key_path:
                ssh_key_path = input(
                    "SSH key path (required for SSH authentication): "
                ).strip()
                if not ssh_key_path:
            ...
            if (
                not ssh_passphrase
                and existing_config
                and existing_config.ssh_passphrase
            ):
                ssh_passphrase = existing_config.ssh_passphrase

        # Proxy Configuration
        proxy_url = input(
            f"HTTP/HTTPS proxy (optional) [{existing_config.proxy_url if existing_config else ''}]: "
        ).strip()
        if not proxy_url:
        ...
        if proxy_url:
            proxy_username = input(
                f"Proxy username (optional) [{existing_config.proxy_username if existing_config else ''}]: "
            ).strip()
            if not proxy_username:
                proxy_username = (
                    existing_config.proxy_username if existing_config else ""
                )
            ...
            if proxy_username:
                ...
                if (
                    not proxy_password
                    and existing_config
                    and existing_config.proxy_password
                ):
...
def check_vcs_client_installed(vcs_type: str) -> bool:
    ...
    try:
        if vcs_type == "git":
            ...
            result = subprocess.run(
                ["hg", "--version"], capture_output=True, text=True, check=True
            )
...
def save_vcs_config(config_manager: VCSConfigManager, config: VCSConfig) -> bool:
    ...
    try:
        ...
        if not check_vcs_client_installed(config.vcs_type):
            print(
                f"❌ {config.vcs_type.upper()} client is not installed or not available in PATH."
            )
            print(
            ...
            response = (
                input(f"\n❓ Continue saving configuration anyway? (y/N): ")
                .strip()
                .lower()
            )
        ...
        with open(ignore_file, "w") as f:
            if vcs_type == "git":
                ...
                for pattern in ignore_patterns:
                    f.write(pattern + "\n")
        print(
            f"✅ Created {os.path.basename(ignore_file)} with patterns: {', '.join(ignore_patterns)}"
        )
    except Exception as e:
...
def execute_vcs_pull(command: str, output_dir: str = None) -> bool:
    ...
    if not config:
        ...
        print(
            "   • Example: vcs-config --vcs-type git --remote-url https://github.com/user/repo.git"
        )
    ...
    if vcs_type == "git":
            ...
            try:
                ...
                subprocess.run(
                    [
                        ...
                        "--local",
                        "user.email",
                        f"{config.username}@adoc-migration-toolkit",
                    ],
                    cwd=target_dir,
                    check=True,
                    capture_output=True,
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                if current_url != config.remote_url:
                    ...
                    subprocess.run(
                        ["git", "remote", "set-url", "origin", config.remote_url],
                        cwd=target_dir,
                        check=True,
                        capture_output=True,
                    )
            ...
            try:
                ...
                try:
                    ...
                    if "untracked working tree files would be overwritten" in error_msg:
                        print(
                            "⚠️  Untracked files conflict detected. Resolving automatically..."
                        )
                        ...
                        conflict_files = re.findall(
                            r"error: The following untracked working tree files would be overwritten by merge:\n\t(.+?)\n",
                            error_msg,
                        )
                        ...
                        if conflict_files:
                            ...
                            backup_dir = os.path.join(
                                target_dir, ".adoc_backup_" + str(int(time.time()))
                            )
                            ...
                            print(
                                f"💾 Backed up {len(conflict_files)} file(s) to: {os.path.basename(backup_dir)}"
                            )
                    ...
                    try:
                        ...
                        error_msg = (
                            merge_error.stderr.decode()
                            if merge_error.stderr
                            else str(merge_error)
                        )
                        ...
                        if "untracked working tree file" in error_msg:
                            ...
                            print(
                                "   1. Check which files are conflicting: 'git status'"
                            )
        ...
        try:
            ...
            if (
                "divergent branches" in error_msg
                or "Need to specify how to reconcile" in error_msg
            ):
                ...
                try:
                    ...
                    try:
                        ...
                        print(
                            "   The repository has divergent branches that need manual resolution."
                        )
                        print("   You can resolve this by:")
                        print("   1. Running 'git status' to see the current state")
                        print(
                            "   2. Choosing a strategy: 'git pull --rebase' or 'git pull --no-rebase'"
                        )
        ...
        if (
            config.username
            and config.token
            and config.remote_url.startswith("https://")
        ):
            auth_section = False
            for i, line in enumerate(hgrc_content):
                if line.strip() == "[auth]":
                    ...
                    for j in range(i + 1, len(hgrc_content)):
                        if hgrc_content[j].startswith("default.username"):
                            ...
                            hgrc_content.insert(
                                j, f"default.username = {config.username}\n"
                            )
                            break
                    else:
...
def execute_vcs_push(command: str, output_dir: str = None) -> bool:
    ...
    if not os.path.exists(target_dir):
        ...
        print(
            "   • Or run 'vcs-init <directory>' to initialize in a specific directory"
        )
        return False
    ...
    if vcs_type == "git":
                ...
                for zip_file in zip_files:
                    try:
                        ...
                        zip_dir = os.path.join(
                            os.path.dirname(zip_file), clean_zip_name
                        )
                        ...
                        print(
                            f"   ✅ Expanded {os.path.basename(zip_file)} → {clean_zip_name}/"
                        )
                        ...
                        print(
                            f"   🧹 Cleaning up directory and file names in {clean_zip_name}/..."
                        )
                            ...
                            for dir_name in dirs:
                                if dir_name.endswith("-import-ready"):
                                    ...
                                    try:
                                        ...
                                        print(
                                            f"      📁 Renamed directory: {dir_name} → {new_dir_name}"
                                        )
                                    except Exception as e:
                                        print(
                                            f"      ⚠️  Could not rename directory {dir_name}: {e}"
                                        )
                        ...
                        print(
                            f"   📋 Created metadata file: {clean_zip_name}/.adoc-migration-meta"
                        )

                        # Remove the original ZIP file after successful extraction
                        os.remove(zip_file)
                        print(
                            f"   🗑️  Removed original ZIP file: {os.path.basename(zip_file)}"
                        )

                    except Exception as e:
                        print(
                            f"   ❌ Error expanding {os.path.basename(zip_file)}: {e}"
                        )
                        continue
        ...
        # Check if there are changes to commit
        status_result = subprocess.run(
            ["hg", "status"], cwd=target_dir, capture_output=True, text=True, check=True
        )
        ...
        if status_result.stdout.strip():
            ...
            # Add all changes
            subprocess.run(
                ["hg", "add"], cwd=target_dir, check=True, capture_output=True
            )
