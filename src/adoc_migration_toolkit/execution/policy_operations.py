...
def execute_policy_list_export(
    client,
    logger: logging.Logger,
    quiet_mode: bool = False,
    verbose_mode: bool = False,
    existing_target_assets_mode: bool = False,
):
    ...
    try:
        ...
        output_file = get_output_file_path(
            "", "policies-all-export.csv", category="policy-export"
        )
        ...
        if not quiet_mode:
            ...
            if existing_target_assets_mode:
                print(
                    f"📋 EXISTING TARGET ASSETS MODE - Only including policies for assets in merged file"
                )
        ...
        if existing_target_assets_mode:
            ...
            if globals.GLOBAL_OUTPUT_DIR:
                merged_file = (
                    globals.GLOBAL_OUTPUT_DIR / "asset-import" / "asset-merged-all.csv"
                )
            else:
                ...
                merged_file = (
                    latest_toolkit_dir / "asset-import" / "asset-merged-all.csv"
                )
        ...
        if verbose_mode:
            print("\nGET Request Headers:")
            print(
                f"  Endpoint: /catalog-server/api/rules?page=0&size=0&ruleStatus=ACTIVE"
            )
            print(f"  Method: GET")
        ...
        for page in range(total_pages):
            ...
            try:
                if verbose_mode:
                    print(f"\nGET Request Headers:")
                    print(
                        f"  Endpoint: /catalog-server/api/rules?page={page}&size={page_size}&ruleStatus=ACTIVE"
                    )
                    print(f"  Method: GET")
                    print(f"  Content-Type: application/json")
                    print(f"  Authorization: Bearer [REDACTED]")
                ...
                if page_response and "rules" in page_response:
                    ...
                    if not quiet_mode:
                        print(
                            f"✅ Page {page + 1}: Retrieved {len(actual_policies)} rules"
                        )
                    else:
                        print(
                            f"✅ Page {page + 1}/{total_pages}: {len(actual_policies)} rules"
                        )

                    successful_pages += 1
                else:
                    error_msg = (
                        f"Invalid response format for page {page + 1} - no rules found"
                    )
                    if not quiet_mode:
                        print(f"❌ {error_msg}")
                    logger.error(error_msg)
        ...
        for i, policy in enumerate(all_policies, 1):
            ...
            if existing_target_assets_mode and table_asset_ids:
                ...
                if verbose_mode:
                    if policy_has_existing_assets:
                        print(
                            f"✅ Processing policy {policy.get('id')}: Found matching assets {matching_assets}"
                        )
                    else:
                        print(
                            f"❌ Skipping policy {policy.get('id')}: No matching assets in target environment (assets: {table_asset_ids})"
                        )
            ...
            if table_asset_ids:
                ...
                if verbose_mode:
                    print(f"\nGET Request Headers:")
                    print(
                        f"  Endpoint: /catalog-server/api/assets/search?ids={table_asset_ids_str}"
                    )
                    print(f"  Method: GET")
                    print(f"  Content-Type: application/json")
                    print(f"  Authorization: Bearer [REDACTED]")
                ...
                try:
                    ...
                    if verbose_mode:
                        print(
                            f"✅ Retrieved details for {len(asset_details)} assets and {len(assembly_details)} assemblies for policy {policy.get('id')}"
                        )
        ...
        if not quiet_mode:
            ...
            if failed_rules:
                ...
                for failed_rule in failed_rules:
                    ...
                    print(
                        f"Table Asset IDs: {', '.join(map(str, failed_rule['table_asset_ids']))}"
                    )
                    ...
                    if table_asset_id:
                        ...
                        if table_asset_id in asset_details:
                            ...
                            if assembly_id and assembly_id in assembly_details:
                                ...
                                source_type = assembly.get("sourceType", {}).get(
                                    "name", ""
                                )
                                if source_type:
        ...
        if not quiet_mode:
            ...
            for policy in processed_policies:
                ...
                engine_type_counts[engine_type] = (
                    engine_type_counts.get(engine_type, 0) + 1
                )
                ...
                for asset in backing_assets:
                    ...
                    if table_asset_id and table_asset_id in asset_details:
                        ...
                        if assembly_id and assembly_id in assembly_details:
                            ...
                            assembly_name = assembly.get("name", "")
                            if assembly_name:
                                assembly_name_counts[assembly_name] = (
                                    assembly_name_counts.get(assembly_name, 0) + 1
                                )
            ...
            if engine_type_counts:
                ...
                print("-" * 50)
                for engine_type, count in sorted(
                    engine_type_counts.items(), key=lambda x: x[1], reverse=True
                ):
            ...
            if assembly_name_counts:
                ...
                for assembly_name, count in sorted(
                    assembly_name_counts.items(), key=lambda x: x[1], reverse=True
                ):
                    percentage = (count / total_assembly_rules) * 100
                    print(
                        f"  {assembly_name:<30} {count:>5} rules ({percentage:>5.1f}%)"
                    )
            else:
            ...
            if source_type_counts:
                ...
                for source_type, count in sorted(
                    source_type_counts.items(), key=lambda x: x[1], reverse=True
                ):
            ...
            print(
                f"✅ Policies list export completed: {len(processed_policies)} policies exported to {output_file}"
            )
...
def execute_policy_list_export_parallel(
    client,
    logger: logging.Logger,
    quiet_mode: bool = False,
    verbose_mode: bool = False,
    ...
    try:
        ...
        if len(all_policies) < min_policies_per_thread:
            ...
            num_threads = min(
                max_threads,
                (len(all_policies) + min_policies_per_thread - 1)
                // min_policies_per_thread,
            )
            policies_per_thread = (len(all_policies) + num_threads - 1) // num_threads

        if not quiet_mode:
            print(
                f"Using {num_threads} threads to process {len(all_policies)} policies"
            )
            ...
            for policy in thread_policies:
                ...
                if existing_target_assets_mode and table_asset_ids:
                    ...
                    if verbose_mode:
                        if policy_has_existing_assets:
                            print(
                                f"✅ Thread {thread_id}: Processing policy {policy.get('id')}: Found matching assets {matching_assets}"
                            )
                        else:
                            print(
                                f"❌ Thread {thread_id}: Skipping policy {policy.get('id')}: No matching assets in target environment (assets: {table_asset_ids})"
                            )
                ...
                if table_asset_ids:
                    ...
                    try:
                        ...
                        failed_asset_calls += 1
                        logger.error(
                            f"Thread {thread_id}: Failed to retrieve asset details for policy {policy.get('id')}: {e}"
                        )
        ...
        if not quiet_mode:
            ...
            for result in thread_results:
                ...
                excluded_info = (
                    f", {result.get('excluded', 0)} excluded"
                    if existing_target_assets_mode
                    else ""
                )
                print(
                    f"Thread {result['thread_id']}: {result['processed']} policies{excluded_info}, "
...
def execute_policy_export(
    ...
    try:
        ...
        if globals.GLOBAL_OUTPUT_DIR:
            input_file = (
                globals.GLOBAL_OUTPUT_DIR / "policy-export" / "policies-all-export.csv"
            )
            ...
            input_file = (
                latest_toolkit_dir / "policy-export" / "policies-all-export.csv"
            )
        ...
        if not input_file.exists():
            ...
            print(f"❌ {error_msg}")
            print(
                f"💡 Please run 'policy-list-export' first to generate the input file"
            )
            logger.error(error_msg)
            return
            ...
            for row_num, row in enumerate(reader, start=2):
                ...
                if export_type == "rule-types":
                    ...
                    if assembly_names:
                        ...
                        assembly_name_list = [
                            name.strip()
                            for name in assembly_names.split(",")
                            if name.strip()
                        ]
                    ...
                    if source_types:
                        ...
                        source_type_list = [
                            stype.strip()
                            for stype in source_types.split(",")
                            if stype.strip()
                        ]
                        if source_type_list:
                ...
                if category:
                    ...
                    logger.warning(
                        f"Row {row_num}: No valid category found for export type '{export_type}'"
                    )
        ...
        if export_type:
            ...
            if filter_value:
                ...
                safe_filter = "".join(
                    c for c in filter_value if c.isalnum() or c in (" ", "-", "_")
                ).rstrip()
        ...
        if not quiet_mode:
            ...
            print(
                f"Found {total_policies} policies across {len(policies_by_category)} {export_type_display}{filter_display}"
            )
        ...
        for policy_type, policy_ids in policies_by_category.items():
            if not quiet_mode:
                ...
                print(
                    f"\033[2F\033[KExporting: [{bar}] {current_batch}/{total_batches} ({percentage:.1f}%)"
                )
                print(
                    f"\033[KStatus: Processing {policy_type} ({len(policy_ids)} policies)"
                )
            else:
            ...
            for batch_num in range(type_total_batches):
                ...
                safe_category = "".join(
                    c for c in policy_type if c.isalnum() or c in (" ", "-", "_")
                ).rstrip()
                safe_category = safe_category.replace(" ", "_").lower()
                batch_filename = (
                    f"{safe_category}-{timestamp}-{start_idx}-{end_idx-1}.zip"
                )
                ...
                try:
                    ...
                    response = client.make_api_call(
                        endpoint=full_endpoint, method="GET", return_binary=True
                    )
                    ...
                    if verbose_mode:
                        ...
                        print(
                            f"Response Size: {len(response) if response else 0} bytes"
                        )
                        if response:
                            print(f"File Size (KB): {len(response) / 1024:.2f} KB")
                            print(
                                f"File Size (MB): {len(response) / (1024 * 1024):.2f} MB"
                            )
                    ...
                    if response:
                        ...
                        error_msg = (
                            f"Empty response for {policy_type} batch {batch_num + 1}"
                        )
                        if verbose_mode:
                            print(f"\n" + "=" * 80)
                    ...
                    error_msg = (
                        f"Failed to export {policy_type} batch {batch_num + 1}: {e}"
                    )
                    if verbose_mode:
                        print(f"\n" + "=" * 80)
                ...
                if not quiet_mode:
                    ...
                    print(
                        f"\033[KStatus: Processing {policy_type} batch {batch_num + 1}"
                    )
                else:
                    print(
                        f"  Batch {batch_num + 1}/{type_total_batches}: {len(batch_ids)} policies"
                    )
                    if response:
        ...
        if not quiet_mode:
            ...
            print(
                f"\033[2F\033[KExporting: [{bar}] {total_batches}/{total_batches} (100.0%)"
            )
        ...
        for policy_type, batch_results in results_by_type.items():
            ...
            for result in batch_results:
                if result["success"]:
                    print(
                        f"    ✅ Batch {result['range']}: {result['count']} policies -> {result['filename']} ({result['file_size']} bytes)"
                    )
                else:
                    print(
                        f"    ❌ Batch {result['range']}: {result['count']} policies -> {result['error']}"
                    )
...
def execute_policy_import(
    ...
    try:
        ...
        if os.path.isabs(file_pattern):
            ...
            search_dir = (
                os.path.dirname(file_pattern) if os.path.dirname(file_pattern) else "."
            )
        else:
            ...
            if not search_dir.exists():
                ...
                print(
                    f"💡 Use 'policy-export' first to generate ZIP files, or specify an absolute path"
                )
                logger.error(error_msg)
                return
        ...
        upload_results = (
            []
        )  # List to store (zip_file, upload_response, upload_uuid) tuples
        ...
        for i, zip_file in enumerate(zip_files, 1):
            ...
            try:
                ...
                if verbose_mode:
                    ...
                    print(
                        f"    File size: {len(file_content)} bytes ({len(file_content) / 1024:.2f} KB)"
                    )
                ...
                if verbose_mode:
                    ...
                    print(
                        f"  Endpoint: /catalog-server/api/rules/import/policy-definitions/upload-config"
                    )
                    print(f"  Method: POST")
                    ...
                    if hasattr(client, "target_tenant") and client.target_tenant:
                        ...
                        print(
                            f"  X-Tenant: {client.tenant} (Source tenant - will be used as target)"
                        )
                ...
                if not upload_uuid:
                    error_msg = (
                        f"Upload config failed - no UUID returned for {zip_file}"
                    )
                    if not quiet_mode:
                        print(f"❌ {error_msg}")
                    logger.error(error_msg)
                ...
                if verbose_mode:
                    ...
                    print(
                        f"  Total Policies: {upload_response.get('totalPolicyCount', 0)}"
                    )
                    print(
                        f"  Data Quality Policies: {upload_response.get('totalDataQualityPolicyCount', 0)}"
                    )
                    print(
                        f"  Data Sources: {upload_response.get('totalDataSourceCount', 0)}"
                    )
                else:
                    print(
                        f"✅ [{i}/{len(zip_files)}] {zip_file}: Upload successful (UUID: {upload_uuid})"
                    )
                ...
                if not quiet_mode:
                    ...
                    if "500" in str(e):
                        print(
                            f"💡 This appears to be a server error (500). Possible causes:"
                        )
                        ...
                        print(
                            f"💡 Request timed out. Try increasing timeout or check network connection."
                        )
                    elif "connection" in str(e).lower():
                        print(
                            f"💡 Connection error. Check network connectivity and server availability."
                        )
                logger.error(error_msg)
                failed_imports += 1
                aggregated_stats["files_failed"] += 1
        ...
        for i, (zip_file, upload_response, upload_uuid) in enumerate(upload_results, 1):
            if not quiet_mode:
                print(
                    f"Applying config {i}/{len(upload_results)}: {zip_file} (UUID: {upload_uuid})"
                )
            ...
            try:
                if verbose_mode:
                    print(f"\n📥 Apply Config")
                    print(
                        f"  Endpoint: /catalog-server/api/rules/import/policy-definitions/apply-config"
                    )
                    print(f"  Method: POST")
                    print(f"  Content-Type: application/json")
                    print(f"  UUID: {upload_uuid}")
                ...
                if apply_response:
                    ...
                    for key in aggregated_stats.keys():
                        if key in upload_response and isinstance(
                            upload_response[key], (int, float)
                        ):
                            if key == "uuids":
                    ...
                    if not quiet_mode:
                        ...
                        print(
                            f"✅ [{i}/{len(upload_results)}] {zip_file}: Apply successful"
                        )
                else:
                    error_msg = (
                        f"Apply config failed for {zip_file} (UUID: {upload_uuid})"
                    )
                    if not quiet_mode:
                        print(f"❌ {error_msg}")
                    logger.error(error_msg)
        ...
        print(
            f"Upload configs successful: {aggregated_stats['upload_configs_successful']}"
        )
        print(f"Upload configs failed: {aggregated_stats['upload_configs_failed']}")
        print(
            f"Apply configs successful: {aggregated_stats['apply_configs_successful']}"
        )
        ...
        if successful_imports > 0:
            ...
            print(
                f"Data Quality Policies: {aggregated_stats['totalDataQualityPolicyCount']}"
            )
            print(f"Data Sources: {aggregated_stats['totalDataSourceCount']}")
            print(f"Business Rules: {aggregated_stats['totalBusinessRules']}")
            print(
                f"Data Cadence Policies: {aggregated_stats['totalDataCadencePolicyCount']}"
            )
            print(
                f"Data Drift Policies: {aggregated_stats['totalDataDriftPolicyCount']}"
            )
            print(
                f"Profile Anomaly Policies: {aggregated_stats['totalProfileAnomalyPolicyCount']}"
            )
            print(
                f"Reconciliation Policies: {aggregated_stats['totalReconciliationPolicyCount']}"
            )
            print(
                f"Schema Drift Policies: {aggregated_stats['totalSchemaDriftPolicyCount']}"
            )
            ...
            print(
                f"Asset UDF Variables: {aggregated_stats['totalAssetUDFVariablesPerAsset']}"
            )

            print(f"\n⚠️  CONFLICTS DETECTED")
            print("-" * 30)
            print(
                f"Conflicting Assemblies: {aggregated_stats['conflictingAssemblies']}"
            )
            ...
            print(
                f"Conflicting Visual Views: {aggregated_stats['conflictingVisualViews']}"
            )
...
def execute_rule_tag_export(
    client, logger: logging.Logger, quiet_mode: bool = False, verbose_mode: bool = False
):
    ...
    try:
        # Determine output file path using the policy-export category
        output_file = get_output_file_path(
            "", "rule-tags-export.csv", category="policy-export"
        )
        ...
        if not policies_file.exists():
            if not quiet_mode:
                ...
                print(
                    "💡 Running policy-list-export first to generate the required file..."
                )
                print("=" * 80)
        ...
        try:
            with open(policies_file, "r", newline="", encoding="utf-8") as f:
                ...
                for row in reader:
                    if (
                        len(row) > 0 and row[0].strip()
                    ):  # First column should be the rule ID
                        try:
        ...
        for i, rule_id in enumerate(rule_ids, 1):
            ...
            try:
                ...
                response = client.make_api_call(
                    endpoint=f"/catalog-server/api/rules/{rule_id}/tags", method="GET"
                )
                ...
                if tag_names:
                    rule_tags_data.append({"rule_id": rule_id, "tag_names": tag_names})
        ...
        if not quiet_mode:
            ...
            if all_tags:
                ...
                if tag_counts:
                    ...
                    for tag_name, count in sorted_tags[:10]:
                        ...
                        print(
                            f"  {tag_name:<30} {count:>5} rules ({percentage:>5.1f}%)"
                        )
            else:
                print(f"\n📊 TAG STATISTICS")
                print("-" * 50)
            ...
            print(
                f"✅ Rule tag export completed: {len(rule_ids)} rules processed, {len(rule_tags_data)} rules with tags written to output"
            )
...
def execute_policy_export_parallel(
    client,
    logger: logging.Logger,
    quiet_mode: bool = False,
    verbose_mode: bool = False,
    ...
    try:
        ...
        if len(policies_by_category) < min_categories_per_thread:
            ...
            num_threads = min(
                max_threads,
                (len(policies_by_category) + min_categories_per_thread - 1)
                // min_categories_per_thread,
            )
            categories_per_thread = (
                len(policies_by_category) + num_threads - 1
            ) // num_threads

        if not quiet_mode:
            print(
                f"Using {num_threads} threads to process {len(policies_by_category)} categories"
            )
                ...
                for batch_num in range(type_total_batches):
                    ...
                    if verbose_mode:
                        ...
                        for k, v in query_params.items():
                            if k == "ids":
                                print(
                                    f"    {k}: {len(batch_ids)} IDs (first few: {', '.join(batch_ids[:3])}{'...' if len(batch_ids) > 3 else ''})"
                                )
                            else:
                                print(f"    {k}: {v}")
                    ...
                    try:
                        ...
                        if verbose_mode:
                            ...
                            print(f"  Content-Type: application/zip")
                            print(
                                f"  File size: {len(response) if response else 0} bytes"
                            )
        ...
        for i in range(num_threads):
            ...
            end_index = min(
                start_index + categories_per_thread, len(policies_by_category)
            )
        ...
        for result in thread_results:
            ...
            print(
                f"{thread_name}: {result['successful_exports']} successful, {result['failed_exports']} failed"
            )
...
def execute_rule_tag_export_parallel(
    client, logger: logging.Logger, quiet_mode: bool = False, verbose_mode: bool = False
):
    ...
    try:
        ...
        if len(rule_ids) < min_rules_per_thread:
            ...
            num_threads = min(
                max_threads,
                (len(rule_ids) + min_rules_per_thread - 1) // min_rules_per_thread,
            )
                ...
                try:
                    ...
                    if tag_names:
                        rule_tags_data.append(
                            {"rule_id": rule_id, "tag_names": tag_names}
                        )
                    ...
                    if verbose_mode:
                        ...
                        print(
                            f"{thread_name} - ✅ Rule {rule_id}: Found {len(tag_names)} tags"
                        )
                        if tag_names:
                            print(f"   Tags: {', '.join(tag_names)}")
                        else:
        ...
        if not quiet_mode:
            ...
            for result in thread_results:
                ...
                print(
                    f"{thread_name}: {result['successful_calls']} successful, {result['failed_calls']} failed, {result['rules_with_tags']} with tags"
                )
            ...
            print(
                f"Rules without tags (skipped): {len(rule_ids) - total_rules_with_tags}"
            )
            ...
            print(
                f"✅ Rule tag export completed: {len(rule_ids)} rules processed, {len(all_rows)} rules with tags written to output"
            )
