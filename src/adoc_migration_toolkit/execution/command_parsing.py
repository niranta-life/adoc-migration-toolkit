...
def parse_segments_export_command(command: str) -> tuple:
    ...
    if not csv_file:
        if globals.GLOBAL_OUTPUT_DIR:
            csv_file = str(
                globals.GLOBAL_OUTPUT_DIR / "policy-export" / "segmented_spark_uids.csv"
            )
        else:
            ...
            if toolkit_dirs:
                ...
                csv_file = str(
                    latest_toolkit_dir / "policy-export" / "segmented_spark_uids.csv"
                )
            else:
...
def parse_asset_profile_export_command(command: str) -> tuple:
    ...
    if not csv_file:
        if globals.GLOBAL_OUTPUT_DIR:
            csv_file = str(
                globals.GLOBAL_OUTPUT_DIR / "asset-export" / "asset_uids.csv"
            )
        else:
            # Look for the most recent adoc-migration-toolkit directory
            current_dir = Path.cwd()
    ...
    while i < len(parts):
        if parts[i] == "--type" and i + 1 < len(parts):
            ...
            if export_type not in [
                ...
                raise ValueError(
                    f"Invalid export type: {export_type}. Must be one of: rule-types, engine-types, assemblies, source-types"
                )
...
def parse_policy_import_command(command: str) -> tuple:
    ...
    for i in range(2, len(parts)):
        if parts[i] == "--quiet" or parts[i] == "-q":
            ...
            print("\nArguments:")
            print(
                "  file_or_pattern: File path or glob pattern (e.g., *.json, data/*.zip)"
            )
            print("\nOptions:")
...
def parse_asset_config_import_command(command: str) -> tuple:
    ...
    while i < len(parts):
        ...
        if arg == "--dry-run":
            ...
            print("=" * 60)
            print(
                "Usage: asset-config-import [<csv_file>] [--dry-run] [--quiet] [--verbose] [--parallel]"
            )
            print("\nArguments:")
            print(
            ...
            print(
                "  --parallel                 Use parallel processing (max 5 threads)"
            )
            print("  --help, -h                 Show this help message")
            print("\nExamples:")
