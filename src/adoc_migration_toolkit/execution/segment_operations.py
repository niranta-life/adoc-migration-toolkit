...
def execute_segments_export(
    ...
    try:
        ...
        if not csv_path.exists():
            ...
            print(f"❌ {error_msg}")
            print(
                f"💡 Please run 'policy-xfr' first to generate the segmented_spark_uids.csv file"
            )
            if globals.GLOBAL_OUTPUT_DIR:
                print(
                    f"   Expected location: {globals.GLOBAL_OUTPUT_DIR}/policy-export/segmented_spark_uids.csv"
                )
            else:
                print(
                    f"   Expected location: adoc-migration-toolkit-YYYYMMDDHHMM/policy-export/segmented_spark_uids.csv"
                )
        ...
        if not output_file:
            output_file = get_output_file_path(
                csv_file, "segments_output.csv", category="policy-import"
            )
            ...
            for i, (source_env, target_env) in enumerate(env_mappings, 1):
                if verbose_mode:
                    ...
                    for j in range(bar_width):
                        if j < filled_blocks:
                            ...
                            asset_index_for_block = int(
                                (j / bar_width) * len(env_mappings)
                            )
                    ...
                    print(
                        f"\033[A\033[KExporting: [{bar}] {i}/{len(env_mappings)} ({percentage:.1f}%)"
                    )
                ...
                try:
                    ...
                    if not quiet_mode:
                        ...
                        print(
                            json.dumps(segments_response, indent=2, ensure_ascii=False)
                        )
        ...
        try:
            with open(output_path, "r", newline="", encoding="utf-8") as f:
                ...
                if len(header) != 2:
                    ...
                    validation_errors.append(
                        f"Invalid header: expected ['target-env', 'segments_json'], got {header}"
                    )
                ...
                for row_num, row in enumerate(reader, start=2):
                    ...
                    if not target_env.strip():
                        validation_errors.append(
                            f"Row {row_num}: Empty target-env value"
                        )

                    if not segments_json_str.strip():
                        validation_errors.append(
                            f"Row {row_num}: Empty segments_json value"
                        )
                        continue
                    ...
                    try:
                        ...
                        if not isinstance(segments_data, dict):
                            validation_errors.append(
                                f"Row {row_num}: segments_json is not a valid JSON object"
                            )
                        elif not segments_data:  # Empty object
                            validation_errors.append(
                                f"Row {row_num}: segments_json is empty"
                            )

                    except json.JSONDecodeError as e:
                        validation_errors.append(
                            f"Row {row_num}: Invalid JSON in segments_json - {e}"
                        )
                    except Exception as e:
                        validation_errors.append(
                            f"Row {row_num}: Error parsing segments_json - {e}"
                        )
        ...
        if not quiet_mode:
            ...
            print(
                f"✅ Segment export completed: {successful} successful, {failed} failed"
            )
    except Exception as e:
...
def execute_segments_import(
    ...
    try:
        ...
        with open(csv_file, "r", newline="", encoding="utf-8") as f:
            ...
            for row_num, row in enumerate(reader, start=2):
                ...
                if target_env and segments_json:
                    ...
                    logger.warning(
                        f"Row {row_num}: Empty target-env or segments_json value"
                    )
        ...
        for i, (target_env, segments_json) in enumerate(import_mappings, 1):
            ...
            try:
                ...
                try:
                    ...
                    if (
                        "assetSegments" in segments_data
                        and "segments" in segments_data["assetSegments"]
                    ):
                        ...
                        error_msg = (
                            f"No 'segments' array found in JSON for UID: {target_env}"
                        )
                        if not quiet_mode:
                            print(f"❌ {error_msg}")
                        logger.error(error_msg)
                    ...
                    if not quiet_mode:
                        ...
                        for seg in import_segments:
                            print(
                                f"  - {seg['name']} ({len(seg['conditions'])} conditions)"
                            )

                except json.JSONDecodeError as e:
                    error_msg = (
                        f"Invalid JSON in segments_json for UID {target_env}: {e}"
                    )
                    if not quiet_mode:
                        print(f"❌ {error_msg}")
                    logger.error(error_msg)
                ...
                if not dry_run:
                    ...
                    if verbose_mode:
                        ...
                        print(
                            f"  Endpoint: /catalog-server/api/assets/{asset_id}/segments"
                        )
                        print(f"  Method: POST")
                        print(f"  Content-Type: application/json")
                        print(f"  Authorization: Bearer [REDACTED]")
        ...
        if not quiet_mode:
            ...
            print(
                f"✅ Segment import completed: {successful} successful, {failed} failed"
            )
            if dry_run:
                print("🔍 DRY RUN MODE - No actual changes were made")
