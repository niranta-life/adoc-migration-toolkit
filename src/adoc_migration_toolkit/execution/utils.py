    ...
    try:
        with open(csv_file, "r", newline="", encoding="utf-8") as f:
            ...
            for row_num, row in enumerate(
                reader, start=2
            ):  # Start at 2 since we skipped header
                if row and len(row) >= 2:
                    ...
                    if source_env and target_env:  # Skip empty values
                        ...
                        logger.debug(
                            f"Row {row_num}: Found source-env: {source_env}, target-env: {target_env}"
                        )
                    else:
                        logger.warning(
                            f"Row {row_num}: Empty source-env or target-env value"
                        )
                else:
                    logger.warning(
                        f"Row {row_num}: Insufficient columns (need at least 2)"
                    )
            ...
            for row_num, row in enumerate(
                ...
                if row and len(row) >= 5:
                    ...
                    if source_id and target_uid:  # Skip rows with empty required fields
                        ...
                        logger.debug(
                            f"Row {row_num}: Found asset - source_id: {source_id}, source_uid: {source_uid}, target_uid: {target_uid}, tags: {tags}"
                        )
                    else:
                        logger.warning(
                            f"Row {row_num}: Empty required fields (source_id or target_uid)"
                        )
                else:
                    logger.warning(
                        f"Row {row_num}: Insufficient columns (need at least 5, got {len(row) if row else 0})"
                    )
