...
including help and command history management.
"""
...
from adoc_migration_toolkit.execution.output_management import (
...
from .asset_operations import (
    ...
    execute_asset_profile_export_parallel,
...
from .policy_operations import (
    ...
    execute_policy_export_parallel,
    execute_policy_import,
    execute_policy_list_export,
    execute_policy_list_export_parallel,
    execute_rule_tag_export,
    execute_rule_tag_export_parallel,
)
...
def log_session_event(logger, event_type: str, user_info: dict = None):
    ...
    try:
        ...
        hostname = user_info.get(
            "hostname", os.uname().nodename if hasattr(os, "uname") else "unknown"
        )
        session_id = user_info.get(
            "session_id", datetime.now().strftime("%Y%m%d_%H%M%S")
        )

        if event_type == "start":
            logger.info(
                f"INTERACTIVE SESSION STARTED - User: {username}, Host: {hostname}, Session: {session_id}"
            )
        elif event_type == "exit":
            logger.info(
                f"INTERACTIVE SESSION EXITED - User: {username}, Host: {hostname}, Session: {session_id}"
            )
...
def show_interactive_help():
    ...
    print(
        f"  {BOLD}segments-export{RESET} [<csv_file>] [--output-file <file>] [--quiet]"
    )
    print("    Export segments from source environment to CSV file")
    print(
        f"  {BOLD}segments-import{RESET} <csv_file> [--dry-run] [--quiet] [--verbose]"
    )
    print("    Import segments to target environment from CSV file")

    print(f"\n{BOLD}🔧 ASSET PROFILE COMMANDS:{RESET}")
    print(
        f"  {BOLD}asset-profile-export{RESET} [<csv_file>] [--output-file <file>] [--quiet] [--verbose] [--parallel]"
    )
    print("    Export asset profiles from source environment to CSV file")
    print(
        f"  {BOLD}asset-profile-import{RESET} [<csv_file>] [--dry-run] [--quiet] [--verbose]"
    )
    print("    Import asset profiles to target environment from CSV file")

    print(f"\n{BOLD}🔍 ASSET CONFIGURATION COMMANDS:{RESET}")
    print(
        f"  {BOLD}asset-config-export{RESET} [<csv_file>] [--output-file <file>] [--quiet] [--verbose] [--parallel]"
    )
    print("    Export asset configurations from source environment to CSV file")
    print(
        f"  {BOLD}asset-config-import{RESET} [<csv_file>] [--dry-run] [--quiet] [--verbose] [--parallel]"
    )
    print("    Import asset configurations to target environment from CSV file")
    print(
        f"  {BOLD}asset-list-export{RESET} [--quiet] [--verbose] [--parallel] [--target] [--page-size <size>]"
    )
    print("    Export all assets from source or target environment to CSV file")
    print(
        f"  {BOLD}asset-tag-import{RESET} [csv_file] [--quiet] [--verbose] [--parallel]"
    )
    print("    Import tags for assets from CSV file")

    print(f"\n{BOLD}📋 POLICY COMMANDS:{RESET}")
    print(
        f"  {BOLD}policy-list-export{RESET} [--quiet] [--verbose] [--parallel] [--existing-target-assets]"
    )
    print("    Export all policies from source environment to CSV file")
    print(
        f"  {BOLD}policy-export{RESET} [--type <export_type>] [--filter <filter_value>] [--quiet] [--verbose] [--batch-size <size>] [--parallel]"
    )
    print(
    ...
    print(
        f'  {BOLD}policy-xfr{RESET} [--input <input_dir>] --string-transform "A":"B", "C":"D", "E":"F" [options]'
    )
    print(
        "    Format policy export files by replacing multiple substrings in JSON files and ZIP archives"
    )
    print(
        f'  {BOLD}transform-and-merge{RESET} --string-transform "A":"B", "C":"D" [--quiet] [--verbose]'
    )
    print("    Transform and merge asset CSV files from source and target environments")

    print(f"\n{BOLD}🔧 VCS COMMANDS:{RESET}")
    print(
        f"  {BOLD}vcs-config{RESET} [--vcs-type <type>] [--remote-url <url>] [--username <user>] [--token <token>] [options]"
    )
    print(
    ...
    print(
        "    Make PUT request to API endpoint with JSON payload (use --target for target environment)"
    )
    print("    Examples:")
...
def show_command_help(command_name: str):
    ...
    print(f"\n" + "=" * 80)
    print(
        f"ADOC INTERACTIVE MIGRATION TOOLKIT - DETAILED HELP FOR: {command_name.upper()}"
    )
    print("=" * 80)
    ...
    if command_name == "segments-export":
        ...
        print(
            "      • Default input: <output-dir>/policy-export/segmented_spark_uids.csv"
        )
        ...
        print("      • If no CSV file specified, uses default from output directory")
        print(
            "      • Default input: <output-dir>/asset-import/asset-profiles-import-ready.csv"
        )
        ...
        print("    Arguments:")
        print(
            "      csv_file: Path to CSV file with 5 columns: source_id, source_uid, target_id, target_uid, tags (optional)"
        )
        print("      --output-file: Specify custom output file (optional)")
        print("      --quiet: Suppress console output, show only summary")
        print("      --verbose: Show detailed output including headers and responses")
        ...
        print("    Behavior:")
        print(
            "      • Reads from asset-import/asset-merged-all.csv by default if no CSV file specified"
        )
        print(
        ...
        print(
            "    Description: Export all assets from source or target environment to CSV file"
        )
        print("    Arguments:")
        print("      --quiet: Suppress console output, show only summary")
        print("      --verbose: Show detailed output including headers and responses")
        ...
        print(
            "      • Parallel mode: Divides pages among threads, combines results, deletes temp files"
        )
        print(
            "      • Target mode: Uses target access key, secret key, and tenant for authentication"
        )
        ...
        print(
            "      • Imports tags for each asset using POST /catalog-server/api/assets/<id>/tag"
        )
        ...
        print("      • Provides comprehensive statistics upon completion")
        print(
            "      • --existing-target-assets: Reads asset-merged-all.csv from asset-import/ directory"
        )
        print(
            "      • --existing-target-assets: Only includes policies where tableAssetId matches source_id in merged file"
        )
        print(
            "      • --existing-target-assets: Optimized with in-memory asset ID lookup"
        )
        print(
        ...
        print("    Behavior:")
        print(
            "      • Reads policies from <output-dir>/policy-export/policies-all-export.csv (generated by policy-list-export)"
        )
        ...
        print(
            "      • Output files: <export_type>[-<filter>]-<timestamp>-<range>.zip in <output-dir>/policy-export"
        )
        ...
        print(
            "      • Reads rule IDs from <output-dir>/policy-export/policies-all-export.csv (first column)"
        )
        print(
        ...
        print(
            "      • Outputs to <output-dir>/policy-export/rule-tags-export.csv with rule ID and comma-separated tags"
        )
        print("      • Shows progress bar in quiet mode")
        print("      • Shows detailed API calls in verbose mode")
        print("      • Provides comprehensive statistics upon completion")
        ...
        print(
            "    Description: Format policy export files by replacing multiple substrings in JSON files and ZIP archives"
        )
        print("    Arguments:")
        ...
        print(
            "      • Replaces all occurrences of multiple source strings with their target strings"
        )
        print("      • Maintains file structure and count")
        print(
            "      • Auto-detects input directory from <output-dir>/policy-export if not specified"
        )
        ...
        print(
            "    Description: Transform and merge asset CSV files from source and target environments"
        )
        print("    Arguments:")
        ...
        print(
            "      • Applies exact string transformations to target_uid column in source file"
        )
        print("      • Only replaces target_uid if it exactly matches a source string")
        print(
            "      • Merges records based on transformed target_uid and target file's source_uid"
        )
        print(
            "      • Merge operation is like an INNER JOIN - only matched records included"
        )
        print("      • Outputs merged file: asset-import/asset-merged-all.csv")
        print(
            "      • Output columns: source_id, source_uid, target_id, target_uid, tags"
        )
        print(
            "      • Only includes records that successfully match between environments"
        )
        print("      • Provides detailed statistics on transformations and matches")
        print(
            "      • Generates <output-dir>/asset-export/asset_uids.csv and <output-dir>/policy-import/segmented_spark_uids.csv"
        )
        print("      • Processes asset-all-export.csv -> asset-all-import-ready.csv")
        print(
            "      • Processes asset-config-export.csv -> asset-config-import-ready.csv"
        )
        ...
        print(
            "    Description: Configure enterprise VCS settings (Git/Mercurial/Subversion, HTTPS/SSH, proxy)"
        )
        print("    Arguments:")
        ...
        print(
            "    Description: Initialize a VCS repository (Git or Mercurial) in the output directory or specified directory."
        )
        print("    Arguments:")
        print(
            "      base directory: Directory to initialize the repository in (optional, defaults to output directory)"
        )
        print("    Behavior:")
        print(
            "      • Initializes a Git or Mercurial repository in the target directory"
        )
        print(
            "      • Creates a .gitignore or .hgignore with patterns: *.zip, config.env, *.log, ~/.adoc_vcs_config.json"
        )
        ...
        print(f"\n{BOLD}vcs-pull{RESET}")
        print(
            "    Description: Pull updates from the configured repository with authentication."
        )
        print("    Behavior:")
        ...
        print(f"\n{BOLD}vcs-push{RESET}")
        print(
            "    Description: Push changes to the remote repository with authentication."
        )
        print("    Behavior:")
        ...
        print(
            "    Description: Change log level dynamically for all loggers in the application"
        )
        print("    Arguments:")
        ...
        print(
            "    Description: Configure HTTP timeout, retry, and proxy settings for all API requests"
        )
        print("    Arguments:")
        ...
        print(
            "    Description: Display current configuration for HTTP, logging, environment, and output settings"
        )
        print("    Arguments:")
        ...
        print(
            "      • Shows access keys and secret keys (partially masked for security)"
        )
        ...
        print("    Arguments:")
        print(
            "      endpoint: API endpoint path (e.g., /catalog-server/api/assets?uid=123)"
        )
...
def run_interactive(args):
    ...
    try:
        ...
        if globals.GLOBAL_OUTPUT_DIR:
            ...
            print(
                f"📁 Output Directory: Not set (will use default timestamped directories)"
            )
            print(
                f"💡 Use 'set-output-dir <directory>' to set a persistent output directory"
            )
        print("=" * 80)
        ...
        while True:
            try:
                ...
                if command.lower().startswith("segments-export"):
                    ...
                    csv_file, output_file, quiet_mode = parse_segments_export_command(
                        command
                    )
                    if csv_file:
                        execute_segments_export(
                            csv_file, client, logger, output_file, quiet_mode
                        )
                    continue
                ...
                if command.lower().startswith("segments-import"):
                    ...
                    csv_file, dry_run, quiet_mode, verbose_mode = (
                        parse_segments_import_command(command)
                    )
                    if csv_file:
                        execute_segments_import(
                            csv_file, client, logger, dry_run, quiet_mode, verbose_mode
                        )
                    continue
                ...
                if command.lower().startswith("asset-profile-export"):
                    ...
                    csv_file, output_file, quiet_mode, verbose_mode, parallel_mode = (
                        parse_asset_profile_export_command(command)
                    )
                    if csv_file:
                        if parallel_mode:
                            execute_asset_profile_export_parallel(
                ...
                if command.lower().startswith("asset-profile-import"):
                    ...
                    csv_file, dry_run, quiet_mode, verbose_mode = (
                        parse_asset_profile_import_command(command)
                    )
                    if csv_file:
                        execute_asset_profile_import(
                            csv_file, client, logger, dry_run, quiet_mode, verbose_mode
                ...
                if command.lower().startswith("asset-list-export"):
                    ...
                    (
                        ...
                        asset_type_ids,
                        assembly_ids,
                    ) = parse_asset_list_export_command(command)
                    if parallel_mode:
                        execute_asset_list_export_parallel(
                            client,
                            logger,
                ...
                if command.lower().startswith("asset-config-export"):
                    ...
                    csv_file, output_file, quiet_mode, verbose_mode, parallel_mode = (
                        parse_asset_config_export_command(command)
                    )
                    if csv_file:
                        if parallel_mode:
                            execute_asset_config_export_parallel(
                ...
                if command.lower().startswith("asset-config-import"):
                    ...
                    csv_file, dry_run, quiet_mode, verbose_mode, parallel_mode = (
                        parse_asset_config_import_command(command)
                    )
                    ...
                    if not csv_file:
                        if globals.GLOBAL_OUTPUT_DIR:
                            ...
                            toolkit_dirs = [
                                ...
                                if d.is_dir()
                                and d.name.startswith("adoc-migration-toolkit-")
                            ]
                            if toolkit_dirs:
                                toolkit_dirs.sort(
                ...
                if command.lower().startswith("asset-tag-import"):
                    ...
                    csv_file, quiet_mode, verbose_mode, parallel_mode = (
                        parse_asset_tag_import_command(command)
                    )
                ...
                if command.lower().startswith("policy-list-export"):
                    ...
                    (
                        ...
                        verbose_mode,
                        parallel_mode,
                        existing_target_assets_mode,
                    ) = parse_policy_list_export_command(command)
                    if parallel_mode:
                        execute_policy_list_export_parallel(
                            client,
                            logger,
                ...
                if command.lower().startswith("policy-export"):
                    ...
                    (
                        ...
                        filter_value,
                        parallel_mode,
                    ) = parse_policy_export_command(command)
                    if parallel_mode:
                        execute_policy_export_parallel(
                            client,
                            logger,
                ...
                if command.lower().startswith("policy-import"):
                    ...
                    file_pattern, quiet_mode, verbose_mode = (
                        parse_policy_import_command(command)
                    )
                    if file_pattern:
                        execute_policy_import(
                            client, logger, file_pattern, quiet_mode, verbose_mode
                        )
                    continue
                ...
                if command.lower().startswith("rule-tag-export"):
                    ...
                    quiet_mode, verbose_mode, parallel_mode = (
                        parse_rule_tag_export_command(command)
                    )
                    if parallel_mode:
                        execute_rule_tag_export_parallel(
                            client, logger, quiet_mode, verbose_mode
                        )
                    else:
                        execute_rule_tag_export(
                            client, logger, quiet_mode, verbose_mode
                ...
                if command.lower().startswith("policy-xfr"):
                    (
                        ...
                        quiet_mode,
                        verbose_mode,
                    ) = parse_formatter_command(command)
                ...
                if command.lower().startswith("transform-and-merge"):
                    ...
                    try:
                        string_transforms, quiet_mode, verbose_mode = (
                            parse_transform_and_merge_command(command)
                        )
                        if string_transforms:
                            execute_transform_and_merge(
                                string_transforms, quiet_mode, verbose_mode, logger
                            )
                    except ValueError as e:
                        print(f"❌ Error: {e}")
                        print(
                            '💡 Usage: transform-and-merge --string-transform "A":"B", "C":"D" [--quiet] [--verbose]'
                        )
                    continue
                ...
                if command.lower().startswith("set-log-level"):
                    ...
                    if new_level:
                        ...
                        if change_log_level(new_level):
                            updated_level = logging.getLevelName(
                                logging.getLogger().getEffectiveLevel()
                            )
                ...
                if command.lower().startswith("show-config"):
                    ...
                    if parse_show_config_command(command):
                        ...
                        if host:
                            ...
                            if "${tenant}" in host:
                                print(
                                    f"  Host Type: Dynamic (uses tenant substitution)"
                                )
                            else:
                        ...
                        if source_access_key:
                            masked_key = (
                                source_access_key[:8] + "..."
                                if len(source_access_key) > 8
                                else source_access_key
                            )
                        ...
                        if source_secret_key:
                            masked_secret = (
                                source_secret_key[:8] + "..."
                                if len(source_secret_key) > 8
                                else source_secret_key
                            )
                        ...
                        if target_access_key:
                            masked_target_key = (
                                target_access_key[:8] + "..."
                                if len(target_access_key) > 8
                                else target_access_key
                            )
                        ...
                        if target_secret_key:
                            masked_target_secret = (
                                target_secret_key[:8] + "..."
                                if len(target_secret_key) > 8
                                else target_secret_key
                            )
                        ...
                        if shared_globals.GLOBAL_OUTPUT_DIR:
                            print(
                                f"  Output Directory: {shared_globals.GLOBAL_OUTPUT_DIR}"
                            )
                        else:
                ...
                if command.lower().startswith("vcs-init"):
                    ...
                    output_dir = (
                        str(globals.GLOBAL_OUTPUT_DIR)
                        if getattr(globals, "GLOBAL_OUTPUT_DIR", None)
                        else None
                    )
                ...
                method, endpoint, json_payload, use_target_auth, use_target_tenant = (
                    parse_api_command(command)
                )
                ...
                if "<asset-id>" in endpoint or "<asset-uid>" in endpoint:
                    ...
                    print(
                        "Please modify the command to replace <asset-id> or <asset-uid> with actual values, then press Enter to continue..."
                    )
                    continue
                ...
                if use_target_auth:
                    ...
                    print(
                        f"   Target Access Key: {client.target_access_key[:8]}..."
                        if client.target_access_key
                        else "   Target Access Key: Not configured"
                    )
                    print(
                        f"   Target Secret Key: {client.target_secret_key[:8]}..."
                        if client.target_secret_key
                        else "   Target Secret Key: Not configured"
                    )
                    print(
                        f"   Target Tenant: {client.target_tenant}"
                        if client.target_tenant
                        else "   Target Tenant: Not configured"
                    )
                    if client.host_template:
                        target_host = client.host_template.replace(
                            "${tenant}", client.target_tenant or "UNKNOWN"
                        )
