...
from adoc_migration_toolkit.execution.asset_operations import execute_transform_and_merge
...
def create_sample_csv_files(asset_export_dir: Path):
    ...
    with open(source_file, 'w', newline='', encoding='utf-8') as f:
        ...
        writer.writerow(['prod-db-1', '1001', 'PROD_DB.prod-db-1', 'database:production'])
        writer.writerow(['prod-db-2', '1002', 'PROD_DB.prod-db-2', 'database:production'])
    ...
    with open(target_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['source_uid', 'source_id', 'target_uid', 'tags'])
        writer.writerow(['DEV_DB.dev-db-1', '101', 'DEV_DB.dev-db-1', 'database:development'])
        writer.writerow(['DEV_DB.dev-db-2', '102', 'DEV_DB.dev-db-2', 'database:development'])
        writer.writerow(['DEV_API.dev-api-1', '201', 'DEV_API.dev-api-1', 'api:development'])
        writer.writerow(['DEV_WEB.dev-web-1', '301', 'DEV_WEB.dev-web-1', 'web:development'])
        ...
        if output_file.exists():
            ...
            with open(output_file, 'r', newline='', encoding='utf-8') as f:
                ...
                for i, row in enumerate(reader, 1):
                    print(f"{i}. Source: {row['source_uid']} -> Target: {row['target_uid']}")
                    print(f"   Source ID: {row['source_id']}, Target ID: {row['target_id']}")
