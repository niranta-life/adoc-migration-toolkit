...
class TestAssetProfileExport:
    ...
    def test_create_progress_bar_disabled(self):
        ...
        progress_bar = create_progress_bar(
            total=10, desc="Test", unit="items", disable=True
        )
    ...
    def test_execute_asset_profile_export_guided_with_progress_bar(
        self, mock_get_output_path, mock_read_csv
    ):
    ...
    @patch("adoc_migration_toolkit.execution.read_csv_uids")
    @patch("adoc_migration_toolkit.execution.get_output_file_path")
    def test_execute_asset_profile_export_guided_verbose_mode(
        self, mock_get_output_path, mock_read_csv
    ):
        """Test that verbose mode disables the progress bar."""
        # Mock the CSV data
        mock_read_csv.return_value = [("asset1", "target1")]
