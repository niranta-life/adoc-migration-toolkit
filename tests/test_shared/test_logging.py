...
class TestSetupLogging:
        ...
        with patch("logging.basicConfig") as mock_basic_config:
            with patch("logging.getLogger") as mock_get_logger:
                ...
                mock_get_logger.assert_called_with(
                    "adoc_migration_toolkit.shared.logging"
                )
    ...
    def test_setup_logging_file_handler_creation(self):
        ...
        with patch("logging.basicConfig") as mock_basic_config:
            with patch("logging.getLogger") as mock_get_logger:
                with patch(
                    "adoc_migration_toolkit.shared.logging.ReadOnlyRotatingFileHandler"
    ...
    def test_setup_logging_formatter_configuration(self):
        ...
        with patch("logging.basicConfig") as mock_basic_config:
            with patch("logging.getLogger") as mock_get_logger:
                with patch(
                    "adoc_migration_toolkit.shared.logging.CustomFormatter"
                ) as mock_formatter:
                    with patch(
                        "adoc_migration_toolkit.shared.logging.ReadOnlyRotatingFileHandler"
                    ) as mock_file_handler:
        ...
        with patch("logging.basicConfig"):
            with patch("logging.getLogger") as mock_get_logger:
                with patch(
                    ...
                    with patch(
                        "adoc_migration_toolkit.shared.logging.datetime"
                    ) as mock_datetime:
                        mock_date = Mock()
