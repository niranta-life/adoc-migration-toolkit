...
class TestAssetTagImport:
    ...
    def test_execute_asset_tag_import_empty_csv(
        ...
        with patch("builtins.print") as mock_print:
            ...
            mock_logger.warning.assert_called_with(
                "No valid asset data found in CSV file"
            )
    ...
    def test_execute_asset_tag_import_sequential_success(
        self, sample_csv_file, mock_client, mock_logger
    ):
        ...
        with patch("builtins.print") as mock_print:
            execute_asset_tag_import(
                str(sample_csv_file), mock_client, mock_logger, quiet_mode=True
            )
            ...
            assert (
                calls[0][1]["endpoint"]
                == "/catalog-server/api/assets?uid=DEV_DB.table1"
            )
            assert calls[0][1]["method"] == "GET"

            # First asset tags
            assert (
                calls[1][1]["endpoint"] == "/catalog-server/api/assets/asset_id_1/tag"
            )
            assert calls[1][1]["method"] == "POST"
            assert calls[1][1]["json_payload"] == {"name": "tag1"}

            assert (
                calls[2][1]["endpoint"] == "/catalog-server/api/assets/asset_id_1/tag"
            )
            assert calls[2][1]["method"] == "POST"
            assert calls[2][1]["json_payload"] == {"name": "tag2"}

    def test_execute_asset_tag_import_sequential_asset_not_found(
        self, sample_csv_file, mock_client, mock_logger
    ):
    ...
    def test_execute_asset_tag_import_sequential_no_asset_id(
        self, sample_csv_file, mock_client, mock_logger
    ):
    ...
    def test_execute_asset_tag_import_sequential_invalid_response(
        self, sample_csv_file, mock_client, mock_logger
    ):
    ...
    def test_execute_asset_tag_import_parallel_success(
        self, sample_csv_file, mock_client, mock_logger
    ):
        ...
        with patch(
            "src.adoc_migration_toolkit.execution.asset_operations.type"
        ) as mock_type:
            ...
            mock_client.make_api_call.side_effect = [
                ...
                # Tag responses
                {"success": True},
                {"success": True},  # asset_id_1 tags
                {"success": True},  # asset_id_2 tag
                {"success": True},
                {"success": True},
                {"success": True},  # asset_id_4 tags
            ]
    ...
    def test_parse_asset_tag_import_command(self):
        ...
        from src.adoc_migration_toolkit.execution.command_parsing import (
            parse_asset_tag_import_command,
        )

        # Test basic command
        csv_file, quiet, verbose, parallel = parse_asset_tag_import_command(
            "asset-tag-import"
        )
        ...
        csv_file, quiet, verbose, parallel = parse_asset_tag_import_command(
            "asset-tag-import test.csv --quiet --parallel"
        )
        assert csv_file == "test.csv"
        assert quiet is True
        assert verbose is False
    ...
    def test_csv_parsing_with_colon_separated_tags(
        self, temp_dir, mock_client, mock_logger
    ):
