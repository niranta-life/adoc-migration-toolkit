...
class TestExecuteAssetProfileExportGuided:
    ...
    def test_execute_asset_profile_export_guided_success(
        self,
        temp_dir,
        mock_client,
        mock_logger,
        ...
        output_files = glob.glob(
            "adoc-migration-toolkit-*/asset-import/asset-profiles-import-ready.csv"
        )
    ...
    def test_execute_asset_profile_export_guided_no_mappings(
        ...
        result, message = execute_asset_profile_export_guided(
            csv_file=str(csv_file), client=mock_client, logger=mock_logger
        )
    ...
    def test_execute_asset_profile_export_guided_api_error(
        self, temp_dir, mock_client, mock_logger, sample_csv_data
    ):
    ...
    def test_execute_asset_profile_export_guided_invalid_response(
        self, temp_dir, mock_client, mock_logger, sample_csv_data
    ):
...
class TestExecuteAssetProfileExport:
    ...
    def test_execute_asset_profile_export_success(
        self,
        temp_dir,
        mock_client,
        mock_logger,
    ...
    def test_execute_asset_profile_export_csv_not_found(self, mock_client, mock_logger):
        ...
        execute_asset_profile_export(
            csv_file="/nonexistent/file.csv", client=mock_client, logger=mock_logger
        )

        mock_client.make_api_call.assert_not_called()
        mock_logger.error.assert_called()

    def test_execute_asset_profile_export_empty_csv(
        self, temp_dir, mock_client, mock_logger
    ):
    ...
    def test_execute_asset_profile_export_with_global_output_dir(
        self,
        temp_dir,
        mock_client,
        mock_logger,
        ...
        with patch(
            "src.adoc_migration_toolkit.execution.asset_operations.globals.GLOBAL_OUTPUT_DIR",
            global_output_dir,
        ):
            execute_asset_profile_export(
                csv_file=str(csv_file), client=mock_client, logger=mock_logger
        ...
        output_files = list(
            global_output_dir.glob("**/asset-profiles-import-ready.csv")
        )
        assert len(output_files) > 0
...
class TestExecuteAssetProfileImport:
    ...
    def test_execute_asset_profile_import_success(
        self, temp_dir, mock_client, mock_logger, sample_asset_response
    ):
        """Test successful asset profile import execution."""
        # Create test CSV file with profile data
        csv_file = temp_dir / "test_profiles.csv"
        profile_data = {
            "profile": {"name": "Test Profile", "config": {"setting": "value"}}
        }
        ...
        assert (
            mock_client.make_api_call.call_count == 2
        )  # Asset details + profile update

    def test_execute_asset_profile_import_dry_run(
        self, temp_dir, mock_client, mock_logger
    ):
        ...
        execute_asset_profile_import(
            csv_file=str(csv_file), client=mock_client, logger=mock_logger, dry_run=True
        )
    ...
    def test_execute_asset_profile_import_invalid_csv_format(
        self, temp_dir, mock_client, mock_logger
    ):
    ...
    def test_execute_asset_profile_import_invalid_json(
        self, temp_dir, mock_client, mock_logger, sample_asset_response
    ):
...
class TestExecuteAssetConfigExport:
    ...
    def test_execute_asset_config_export_success(
        self, temp_dir, mock_client, mock_logger, sample_config_response
    ):
        ...
        assert (
            mock_client.make_api_call.call_count == 2
        )  # 2 assets * 1 config call each

        # Verify output file was created in global output directory
        import glob

        output_files = glob.glob(
            "adoc-migration-toolkit-*/asset-export/asset-config-export.csv"
        )
        assert len(output_files) > 0

    def test_execute_asset_config_export_no_assets(
        self, temp_dir, mock_client, mock_logger
    ):
    ...
    def test_execute_asset_config_export_api_error(
        self, temp_dir, mock_client, mock_logger
    ):
...
class TestExecuteAssetConfigImport:
    ...
    def test_execute_asset_config_import_success(
        self, temp_dir, mock_client, mock_logger
    ):
        ...
        expected_payload_1 = {"assetConfiguration": {"assetId": 123, "config": "data1"}}
        ...
        expected_payload_2 = {"assetConfiguration": {"assetId": 456, "config": "data2"}}
        assert call_args[1]["json_payload"] == expected_payload_2

    def test_execute_asset_config_import_csv_not_found(
        self, temp_dir, mock_client, mock_logger
    ):
    ...
    def test_execute_asset_config_import_no_assets(
        self, temp_dir, mock_client, mock_logger
    ):
    ...
    def test_execute_asset_config_import_asset_not_found(
        self, temp_dir, mock_client, mock_logger
    ):
        ...
        assert (
            ...
            == "/catalog-server/api/assets?uid=nonexistent.asset"
        )
...
class TestExecuteAssetConfigExportParallel:
    ...
    def test_execute_asset_config_export_parallel_success(
        self, temp_dir, mock_client, mock_logger
    ):
        ...
        with open(csv_file, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["source_id", "source_uid", "target_id", "target_uid", "tags"]
            )
        ...
        with patch(
            ...
            execute_asset_config_export_parallel(
                str(csv_file), mock_client, mock_logger, quiet_mode=True
            )
    ...
    def test_execute_asset_config_export_parallel_no_assets(
        self, temp_dir, mock_client, mock_logger
    ):
    ...
    def test_execute_asset_config_export_parallel_api_error(
        self, temp_dir, mock_client, mock_logger
    ):
    ...
    def test_parallel_mode_defaults_to_quiet(self, temp_dir, mock_client, mock_logger):
        ...
        from src.adoc_migration_toolkit.execution.command_parsing import (
            parse_asset_config_export_command,
        )
        ...
        with (
            patch(
                "src.adoc_migration_toolkit.execution.command_parsing.globals.GLOBAL_OUTPUT_DIR",
                temp_dir,
            ),
...
class TestExecuteAssetListExport:
    ...
    def test_execute_asset_list_export_success(
        self, temp_dir, mock_client, mock_logger
    ):
        """Test successful asset list export execution."""
        # Mock count response
        count_response = {"meta": {"count": 5}}
        ...
        page_response = {
            "data": {
                "assets": [
                    ...
                    {"asset": {"id": 2, "uid": "asset-2"}, "tags": [{"name": "tag3"}]},
                    {"asset": {"id": 3, "uid": "asset-3"}, "tags": []},
                ]
            }
        }
    ...
    def test_execute_asset_list_export_with_autotagged_filtering(
        self, temp_dir, mock_client, mock_logger
    ):
        """Test asset list export with autoTagged tag filtering."""
        # Mock count response
        count_response = {"meta": {"count": 3}}
    ...
    def test_execute_asset_list_export_no_assets(
        ...
        with patch(
            ...
            execute_asset_list_export(client=mock_client, logger=mock_logger)

        # Verify output file was created (even with no assets)
        output_file = temp_dir / "asset-export" / "asset-all-source-export.csv"
        assert output_file.exists()

    def test_execute_asset_list_export_api_error(
        self, temp_dir, mock_client, mock_logger
    ):
    ...
    def test_execute_asset_list_export_invalid_count_response(
        self, temp_dir, mock_client, mock_logger
    ):
    ...
    def test_execute_asset_list_export_without_global_output_dir(
        self, temp_dir, mock_client, mock_logger
    ):
        """Test asset list export without global output directory."""
        # Mock count response
        count_response = {"meta": {"count": 1}}
        ...
        mock_client.make_api_call.side_effect = [count_response, page_response]
    ...
    def test_execute_asset_list_export_alternative_response_structure(
        self, temp_dir, mock_client, mock_logger
    ):
        """Test asset list export with alternative response structure."""
        # Mock count response
        count_response = {"meta": {"count": 2}}

        # Mock page response with different structure
        page_response = {
            "data": {
                "items": [{"id": 1, "uid": "asset-1"}, {"id": 2, "uid": "asset-2"}]
            }
        }
    ...
    def test_execute_asset_list_export_parallel_success(
        self, temp_dir, mock_client, mock_logger
    ):
        """Test successful parallel asset list export execution."""
        # Mock count response
        count_response = {"meta": {"count": 10}}
        ...
        with (
            ...
            execute_asset_list_export_parallel(
                client=mock_client, logger=mock_logger, verbose_mode=False
            )
    ...
    def test_execute_asset_list_export_parallel_with_autotagged_filtering(
        self, temp_dir, mock_client, mock_logger
    ):
        """Test parallel asset list export with autoTagged tag filtering."""
        # Mock count response
        count_response = {"meta": {"count": 6}}
...
class TestAssetOperationsIntegration:
    ...
    def test_asset_profile_export_import_workflow(
        self,
        temp_dir,
        mock_client,
        mock_logger,
    ...
    def test_asset_config_export_import_workflow(
        self, temp_dir, mock_client, mock_logger, sample_config_response
    ):
        ...
        mock_client.make_api_call.side_effect = [sample_config_response]
        ...
        execute_asset_config_import(
            csv_file=str(import_csv), client=mock_client, logger=mock_logger
        )
    ...
    def test_error_handling_across_operations(self, temp_dir, mock_client, mock_logger):
        ...
        for csv_file, operation in test_cases:
            if operation == "asset-profile-export":
                execute_asset_profile_export(
                    csv_file=csv_file, client=mock_client, logger=mock_logger
                )
    ...
    def test_parse_asset_list_export_command_with_page_size(self):
        ...
        from src.adoc_migration_toolkit.execution.command_parsing import (
            parse_asset_list_export_command,
        )
        ...
        (
            ...
            asset_type_ids,
            assembly_ids,
        ) = parse_asset_list_export_command("asset-list-export --page-size 1000")
        assert not quiet
        assert not verbose
        assert not parallel
        assert not target
        ...
        (
            ...
            "asset-list-export --page-size 250 --verbose --parallel"
        )
        assert not quiet
        assert verbose
        assert parallel
        ...
        (
            ...
            "asset-list-export --target --page-size 750 --quiet"
        )
        assert quiet
        assert not verbose
        assert not parallel
        ...
        (
            ...
            asset_type_ids,
            assembly_ids,
        ) = parse_asset_list_export_command("asset-list-export")
        assert not quiet
        assert not verbose
        assert not parallel
        assert not target
    ...
    def test_parse_transform_and_merge_command(self):
        ...
        from src.adoc_migration_toolkit.execution.command_parsing import (
            parse_transform_and_merge_command,
        )

        # Test with string transformations
        string_transforms, quiet_mode, verbose_mode = parse_transform_and_merge_command(
        ...
        string_transforms, quiet_mode, verbose_mode = parse_transform_and_merge_command(
            'transform-and-merge --string-transform "A":"B", "C":"D" --verbose'
        )
        ...
        string_transforms, quiet_mode, verbose_mode = parse_transform_and_merge_command(
            'transform-and-merge --string-transform "old":"new" --quiet'
        )
    ...
    def test_execute_transform_and_merge_success(self, temp_dir, mock_logger):
        ...
        with open(target_file, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow(["source_uid", "source_id", "target_uid", "tags"])
            writer.writerow(
                ["snowflake_krish_test", "101", "snowflake_krish_test", "tag1:tag2"]
            )
        ...
        with patch(
            ...
            string_transforms = {
                "Snowflake": "snowflake_krish_test",
                "SNOWFLAKE_SAMPLE_DATA": "KRISH_TEST",
            }
