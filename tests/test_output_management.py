...
class TestParseSetOutputDirCommand:
    ...
    def test_invalid_path(self):
        with patch('pathlib.Path.resolve', side_effect=Exception('bad path')):
            assert output_management.parse_set_output_dir_command('set-output-dir /bad/path') is None
...
class TestSetGlobalOutputDirectory:
    def test_sets_and_saves(self, temp_dir, mock_logger):
        result = output_management.set_global_output_directory(str(temp_dir), mock_logger)
        assert result is True
        assert output_management.globals.GLOBAL_OUTPUT_DIR.resolve() == temp_dir.resolve()
    def test_creates_directory(self, temp_dir, mock_logger):
        new_dir = temp_dir / "new"
        result = output_management.set_global_output_directory(str(new_dir), mock_logger)
        assert result is True
        assert new_dir.exists()
    def test_invalid_directory(self, temp_dir, mock_logger):
        file_path = temp_dir / "afile.txt"
        file_path.write_text("hi")
        result = output_management.set_global_output_directory(str(file_path), mock_logger)
        assert result is False
    def test_cannot_create(self, temp_dir, mock_logger):
        with patch('pathlib.Path.mkdir', side_effect=Exception('fail')):
            result = output_management.set_global_output_directory(str(temp_dir / 'bad'), mock_logger)
            assert result is False
