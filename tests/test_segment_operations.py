...
def sample_asset_response():
    return {"data": [{"id": 12345, "uid": "asset-1-PROD_DB", "name": "Test Asset 1"}]}
            ...
            "segments": [
                {
                    "name": "seg1",
                    "conditions": [
                        {"columnId": 1, "condition": "EQUALS", "value": "foo"}
                    ],
                },
...
class TestExecuteSegmentsExport:
    def test_success(
        ...
        print(
            f"Segments length: {len(segments) if isinstance(segments, list) else 'N/A'}"
        )

        # Patch globals in both modules to ensure output file is created in temp directory
        with (
            patch(
                "src.adoc_migration_toolkit.execution.segment_operations.globals.GLOBAL_OUTPUT_DIR",
                temp_dir,
            ),
            patch(
                "src.adoc_migration_toolkit.shared.file_utils.globals.GLOBAL_OUTPUT_DIR",
                temp_dir,
            ),
        ):
            execute_segments_export(
...
class TestExecuteSegmentsImport:
    def test_success(
        self, temp_dir, mock_client, mock_logger, sample_segments_response
    ):
